...
module.exports = {
  data: new SlashCommandBuilder()
    ...
    async execute(interaction) {
      ...
      let results = await Promise.all([getSyndictResults(searchTerm), getMoedictResults(tify(searchTerm))])
      ...
      const buttonRow = new ActionRowBuilder()
        .addComponents(changeDictionaryBtn)
      ...
      */
      ...
      const embedResponse = new EmbedBuilder()
        ...
        .setFooter({ text: `Page ${currentPage + 1} of ${results[currentDictionaryIndex].length}` })

      if (results[currentDictionaryIndex].length > 1) {
        buttonRow.addComponents(nextPageBtn)
      } 
      
      await interaction.editReply({ embeds: [embedResponse], components: [buttonRow] })
      ...
      collector.on("collect", async int => {
        ...
        embedResponse.setFooter({ text: `Page ${currentPage + 1} of ${results[currentDictionaryIndex].length}` })
